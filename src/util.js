...
export function tagsMatchFilter(tags, filter) {
  ...
  // Find a group where not all of the checked tags from filter is in tags
  const groupWithNoCheckedTags = groups.find((group) => {
    ...
    // Exclude all lessons that don't have identical tags to the filter
    return arrayIntersection(tagNamesInFilter, tagNamesInTags).length !== tagNamesInFilter.length;
  });
